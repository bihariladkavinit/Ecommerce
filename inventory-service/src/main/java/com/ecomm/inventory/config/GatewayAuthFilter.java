 ...
 * Internal Feign calls (e.g. from Cart Service for availability checks) won't
 * carry them — those paths are permit-all in {@link SecurityConfig}.
 *
 * <p>The {@code X-User-Id} is also injected into the MDC as {@code userId} and
 * {@code traceId} so all log lines for a request are correlated to the caller.
 *
 * <p><strong>Security note:</strong> these headers must only be trusted from
 * requests originating via the Gateway. The Docker network configuration ensures
 * inventory-service is not reachable directly from outside the cluster.
 */
@Slf4j
