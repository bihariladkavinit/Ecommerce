...
/**
 * Reads the trusted internal headers forwarded by the API Gateway after it has
 ...
 * </ul>
 *
 * <p>Sets the principal as a {@link UUID} so {@code SecurityContextHolder.getContext()
 * .getAuthentication().getPrincipal()} returns a UUID directly in the service layer.
 *
 * <p>Also injects {@code userId} and {@code traceId} into the MDC so every log line
 * for a request is correlated to the caller.
 */
@Slf4j
