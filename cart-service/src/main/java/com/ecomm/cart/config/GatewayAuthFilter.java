 ...
 * </ul>
 *
 * <p>The Gateway is the sole public entry point. Requests that reach this
 * service without these headers (e.g. internal Feign calls from other services
 * that don't forward a JWT) fall through with no authentication set — those
 * paths must be explicitly permit-all in {@link SecurityConfig}.
 *
 * <p>The {@code X-User-Id} is also injected into the MDC as {@code userId} and
 * {@code traceId} so every log line for a request is correlated to the caller.
 */
@Slf4j
